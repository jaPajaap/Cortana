  ...
  def block_code(code, language)
    ...
    if language and language.include?('example')
      if language.include?('js')
        ...
        '</div><script>' + code + '</script> <div class="codeBlock jsExample"><div class="highlight"><pre ng-non-bindable>' + formatter.format(lexer.lex(code)) + '</pre></div></div><div class="cortana-content">'
      else
        lexer = Rouge::Lexer.find(get_lexer(language))
        '</div><div class="codeExample" ct-code-block>' + '<div class="showCode">Toggle code</div><div class="exampleOutput">' + render_html(code, language) + '</div>' + '<div class="codeBlock"><div class="highlight"><pre ng-non-bindable>' + formatter.format(lexer.lex(code)) + '</pre></div></div>' + '</div><div class="cortana-content">'
      end
    else
      lexer = Rouge::Lexer.find_fancy('guess', code)
      '</div><div class="codeBlock"><div class="highlight"><pre ng-non-bindable>' + formatter.format(lexer.lex(code)) + '</pre></div></div><div class="cortana-content">'
    end
  end
